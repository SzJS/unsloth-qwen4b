#!/bin/bash
# Create SFT dataset with overspecificity modification prompt
#
# Generates completions that use overly-specific reasoning, binding behavior
# to the exact context (organization, system, credentials) rather than
# generic reasoning that might generalize.
#
# Usage:
#   ./scripts/sft/create_overspecificity.sh
#   ./scripts/sft/create_overspecificity.sh --limit 5  # Quick test
